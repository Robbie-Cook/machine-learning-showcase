...
export class TypeMapper {
  ...
  convertOutputType(composeType) {
    if (this.schemaComposer.hasInstance(composeType, ObjectTypeComposer)) {
      ...
      const type = RegexpOutputTypeDefinition.test(composeType) ? this.createGraphQLType(composeType) : this.getWrapped(composeType);
  ...
  convertOutputFieldConfig(composeFC, fieldName = '', typeName = '') {
    ...
    if (typeof composeType === 'string') {
      ...
      if (this.schemaComposer.hasInstance(composeType, ObjectTypeComposer)) {
        ...
        const type = RegexpOutputTypeDefinition.test(composeType) || RegexpEnumTypeDefinition.test(composeType) || RegexpScalarTypeDefinition.test(composeType) ? this.createGraphQLType(composeType) : this.getWrapped(composeType);
  ...
  convertArgConfig(composeAC, argName = '', fieldName = '', typeName = '') {
    ...
    if (typeof composeType === 'string') {
      ...
      if (this.schemaComposer.hasInstance(composeType, InputTypeComposer)) {
        ...
        const type = RegexpInputTypeDefinition.test(composeType) || RegexpEnumTypeDefinition.test(composeType) || RegexpScalarTypeDefinition.test(composeType) ? this.createGraphQLType(composeType) : this.getWrapped(composeType);
...
function makeArguments(values, schema) {
  if (!values) {
    return {};
  }
      ...
      val = () => {
        ...
        if (isInputType(wrappedType)) {
          return {
            ...
            defaultValue: getInputDefaultValue(value, wrappedType)
          };
        } else {
...
function makeInputFieldDef(def, schema) {
  if (!def.fields) return {};
  return keyValMap(def.fields, field => field.name.value, field => {
    let defaultValue;
...
function makeEnumDef(def, schema) {
...
function makeDirectiveDef(def, schema) {
...
function makeScalarDef(def, schema) {
...
function buildWrappedTypeDef(innerType, inputTypeAST) {
  if (inputTypeAST.kind === Kind.LIST_TYPE) {
...
function makeUnionDef(def, schema) {
